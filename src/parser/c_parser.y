...
%type<p_ast_base_statement> statement labeled_statement
%type<p_if_else_statement> if_else_statement
%type<p_loop_statement> for_while_statement
%type<p_ast_return_statement> return_statement
...
top_declaration           : top_declaration declaration                                         {$$->add_new($2);               }
                          | top_declaration function_definition                                 {$$->add_new($2);               }
                          | top_declaration Semicolon
                          | declaration                                                              {$$ = new ast_top(); $$->add_new($1);               }
...
declarator                : Open_BRACKET declarator Close_BRACKET                                    { $$ = $2; }
                          | IDENTIFIER OSquare_BRACKET constant_expression CSquare_BRACKET           { ; }
                          | IDENTIFIER OSquare_BRACKET CSquare_BRACKET                               { ; }
...
initialisers_declarator   : declarator                                                            { $$ = new ast_initialisers_declarator($1, NULL);    }
...
postfix_expression        : primary_expression                                                      { $$ = $1;  }
                          ...
                          | primary_expression Open_BRACKET Close_BRACKET                           { $$ = new ast_function_call($1);}
                          | primary_expression Open_BRACKET argument_expression_list Close_BRACKET  { $$ = new ast_function_call($1, *$3);}
                          | primary_expression Dot_Ref IDENTIFIER 
                          | primary_expression Increment_MATH                                       { $$ = new ast_add_operator($1, new ast_expression_number( new ast_number(1) ));  }
                          | primary_expression Decrement_MATH                                       { $$ = new ast_subtract_operator($1, new ast_expression_number( new ast_number(1) ));  }
                          ;

unary_expression          : postfix_expression                                                      { $$ = $1; }
                          | Increment_MATH unary_expression                                         { $$ = new ast_add_operator($2, new ast_expression_number( new ast_number(1) ));  }
                          | Decrement_MATH unary_expression                                         { $$ = new ast_subtract_operator($2, new ast_expression_number( new ast_number(1) ));  }
                          | unary_operator cast_expression                                          { $$ = $2;  }
                          | Sizeof_KEY unary_expression                                             { $$ = $2; }
                          | Sizeof_KEY Open_BRACKET type_name Close_BRACKET                         {; }
                          ;
...
if_else_statement         : If_KEY Open_BRACKET expression Close_BRACKET statement { $$ = new ast_if_else_statement($3); $$->push_back_if($5); $$->push_back_else(NULL); }
                          ...
                          | Switch_KEY Open_BRACKET expression Close_BRACKET statement {;}
                          ;
...
return_statement          : Continue_KEY Semicolon                        {}
                          | Break_KEY Semicolon                           {}
                          | Return_KEY Semicolon                          {$$ = new ast_return_statement(NULL);}
